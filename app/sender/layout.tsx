...
const senderLinks = [
  { href: "/sender/dashboard", label: "Operations" },
  { href: "/sender/shipment/create", label: "New Shipment" },
  { href: "/sender/dispute", label: "Disputes" },
];
...
export default function SenderLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (isReady && (!user || user.role !== "SENDER")) {
      router.push("/login");
    }
  }, [user, isReady, router]);

  if (!isReady || !user || user.role !== "SENDER") {
    return <div className="flex min-h-screen items-center justify-center">Loading Sender Console...</div>;
  }
      ...
      <div className="flex w-full max-w-[1400px] border border-slate-200 bg-white shadow-sm overflow-hidden flex-col md:flex-row">
        ...
        <aside className="w-full md:w-[300px] border-b md:border-b-0 md:border-r border-slate-200 flex flex-col shrink-0">
          <div className="p-10 border-b border-slate-200">
            ...
            <div className="text-xs font-bold text-slate-400 uppercase tracking-widest">Guest Check</div>
            <div className="text-[10px] text-slate-400 italic">every shipment counts</div>
          </div>
          
          <nav className="flex-grow">
            <ul className="list-none">
              {senderLinks.map((link) => (
          ...
          <div className="p-8 border-t border-slate-200">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Authorized By</span>
