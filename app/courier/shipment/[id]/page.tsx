...
import { CourierActions } from "@/components/courier/CourierActions";
import { use, useState, useEffect } from "react";
import { api } from "@/lib/api";
import { Shipment } from "@/types";

export default function CourierShipmentPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#EAEBEB]">
      ...
      <div className="px-6 lg:px-12 py-8 lg:py-10 border-b border-black bg-white relative">
        <div className="flex flex-wrap items-center gap-2 lg:gap-3 mb-4 lg:mb-6">
          ...
          <span className="text-[8px] lg:text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            PROTO: {id.slice(0, 14).toUpperCase()}
          </span>
        </div>
      ...
      <div className="flex-grow p-6 lg:p-12">
        <ShipmentDetail shipmentId={id} role="COURIER" hideHeader={true}>
          {shipment && <CourierActions shipmentId={id} shipmentStatus={shipment.status} />}
        </ShipmentDetail>
      </div>
    </div>
  );
