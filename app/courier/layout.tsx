...
const courierLinks = [
  { href: "/courier/dashboard", label: "My Deliveries" },
];
...
export default function CourierLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (isReady && (!user || user.role !== "COURIER")) {
      router.push("/login");
    }
  }, [user, isReady, router]);

  if (!isReady || !user || user.role !== "COURIER") {
    return <div className="flex min-h-screen items-center justify-center">Loading Courier Console...</div>;
  }
      ...
      <div className="flex w-full max-w-[1400px] md:border md:border-slate-200 bg-white md:shadow-sm overflow-hidden flex-col md:flex-row">
        ...
        <aside className="w-full md:w-[300px] border-b md:border-b-0 md:border-r border-slate-200 flex flex-col shrink-0">
          <div className="p-10 border-b border-slate-200">
            ...
            <div className="text-xs font-bold text-slate-400 uppercase tracking-widest">Driver Check</div>
            <div className="text-[10px] text-slate-400 italic">delivery protocol active</div>
          </div>
          ...
          <div className="p-8 border-t border-slate-200">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Authenticated Driver</span>
